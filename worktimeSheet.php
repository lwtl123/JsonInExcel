...
class Transform extends Input
{
    ...
    public function transformData(){
        ...
        $this->month = date_format(new DateTime($data[0]["Started"]), 'M');
...
{
    public function setSheetFormat()
    {
    ...
    public function setPdfFormat () {
        $sheet = $this->sheet;
        $timeData = $this->timeData;
        $spreadsheet = $this->spreadsheet;

        for ($startCellPDF = 8; $startCellPDF < count($timeData) + 8; $startCellPDF++) {
...
class OutputFile extends Format
{
    public function getOutputFilesFrom ($insertFile){
        $this->read($insertFile);
        $this->transformData();
        $this->setSheetFormat();
        $this->getExcelSheet();
        $this->getPdfSheet();
    ...
    public function getExcelSheet(){
        ...
        $writer->save($this->month . '. Arbeitszeiten.xlsx');
    }

    public function getPdfSheet(){
        $this->setPdfFormat();
        $pdf = new \PhpOffice\PhpSpreadsheet\Writer\Pdf\DOMPDF($this->spreadsheet);
        $pdf->save($this->month . '. Arbeitszeiten.pdf');
    }

}

$ob=new OutputFile();
$ob->getOutputFilesFrom($argv[1]); //oder hier den Dateipfad angeben

/*
PhpUnit nutzen
Klassen Format und Outputfile sind mit mehrfacher Zuständigkeit
->mit Composite/Adapter lösen
->Aufteilung in Format, PdfFormat, OutputExcel, OutputPdf
